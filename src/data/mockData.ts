...
// Generate validation messages dynamically from indicator data
export const generateValidationMessages = (): ValidationMessage[] => {
  const messages: ValidationMessage[] = [];
  
  Object.entries(indicatorDataMap).forEach(([indicatorId, questions]) => {
    questions.forEach((question) => {
      if (question.validation === 'error') {
        messages.push({
          id: `${indicatorId}-${question.code}`,
          type: 'error',
          code: question.code,
          message: `${question.code} missing (required)`,
          indicatorId,
        });
        ...
        messages.push({
          ...
          type: 'warning',
          code: question.code,
          message: `${question.code} – please review and verify`,
          indicatorId,
        });
      }
    });
